...
class Solution {
    ...
    /*
        ...
        노드별 승리 데이터셋을 기준으로 최종 승리 노드셋 계산 ( 자기 자신은 항상 포함, 괄호는 가상으로 만들어진 노드셋 )
        1 = [1, 2, (5)]
        2 = [2, 5]
        3 = [2, (3), (5)]
        4 = [(2), (3), 4, 5]
        ...
        노드별 패배 데이터셋을 기준으로 최종 패배 노드셋 계산 ( 자기 자신은 항상 포함, 괄호는 가상으로 만들어진 노드셋 )
        1 = [1]
        2 = [1, 2, 3, 4]
        3 = [3, 4]
        4 = [4]
        ...
        승리 데이터 셋과 패배 데이터 셋을 합쳤을 경우 노드셋 계산
        1 = [1, 2, (5)]
        2 = [1, 2, 3, 4, 5]
        3 = [2, (3), 4, (5)]
        4 = [(2), (3), 4, 5]
    ...
    private Queue<Integer> queue = new LinkedList<>();                  // BFS 큐
    private Set<Integer> tempSet = new HashSet<>();                     // BFS 내에서 사용되는 임시 셋
    private boolean[] isVisit = null;                                   // 방문 배열
    ...
    public void bfs(Integer node, Map<Integer, Set<Integer>> scoreMap){
        ...
        while(!queue.isEmpty()){
            ...
            for(Integer iNode : scoreMap.get(queueNode)){
                // 방문 체크
                if(!isVisit[iNode - 1]){
    ...
    public int solution(int n, int[][] results) {
        // 노드 개수 할당
        nodeCount = n;
        // 방문 배열 초기화 (예전 속도 이슈로 방문리스트 -> 방문 배열로 변경)
        isVisit = new boolean[nodeCount];
        ...
        // 2차 승리, 패배 데이터 셋 구축 ( 각자 for 문에서 리팩토링 )
